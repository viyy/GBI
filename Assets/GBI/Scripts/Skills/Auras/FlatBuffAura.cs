...
namespace Geekbrains.Skills.Auras
{
    ...
    {
        public FlatBuffAura(int id, AuraTypes type, string name, bool isVisible, bool isPermanent, float duration,
        ...
        {
            foreach (var f in Values) target.AddToCharacteristic(f.Key, Mathf.FloorToInt(f.Value));
        }
